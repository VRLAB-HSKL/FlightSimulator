...
{
    [SerializeField] public MotorController m_MotorController;
    ...
    private Vector3 GForce; // Forces that impact the pilot, used for setting motor 
    private Acceleration state = Acceleration.Constant;
    ...
    private void CalculateLinearForces()
    {
        ...
        float liftPower = ForwardSpeed * ForwardSpeed * m_Lift * zeroLiftFactor * m_AeroFactor;
    ...
    private void SetMotors()
    {
        try
        {
            if (!m_MotorController.connected())
            {
                return;
            }
        }
        catch (NullReferenceException isNull)
        {
            return;
        }
        
        float yAxis = GForce.y;
        float zAxis = GForce.z;

        switch (state)
        {
            case Acceleration.Accelerating:
                m_MotorController.setMotor1();
                break;
            case Acceleration.Constant:
                m_MotorController.disableMotor1();
                break;
            case Acceleration.Decelerating:
